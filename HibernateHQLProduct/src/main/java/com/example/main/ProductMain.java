...
public class ProductMain {
    ...
    public static void main(String[] args) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // Insert Products (run once, then comment out)
            ...
            Object[] minMax = session.createQuery(
                    "SELECT MIN(p.price), MAX(p.price) FROM Product p", Object[].class).uniqueResult();
            ...
            // WHERE + LIKE
            // ===============================
            List<Product> likeList = session.createQuery(
                    "FROM Product p WHERE p.name LIKE 'L%'", Product.class).list();
            System.out.println("\n--- Names starting with L ---");
            likeList.forEach(p -> System.out.println(p.getName()));

            List<Product> endWithList = session.createQuery(
                    "FROM Product p WHERE p.name LIKE '%p'", Product.class).list();
            System.out.println("\n--- Names ending with 'p' ---");
            endWithList.forEach(p -> System.out.println(p.getName()));

            List<Product> containList = session.createQuery(
            ...
            List<Product> lengthList = session.createQuery(
                    "FROM Product p WHERE LENGTH(p.name) = 5", Product.class).list();
